...
int ike_initiate(ike_context_t *ctx, uint8_t *resp, uint16_t *resp_len) {
    if (ctx->role != IKE_ROLE_INITIATOR) return -1;
    ike_header_t *hdr = (ike_header_t *)resp;
    uint8_t *payload = resp + sizeof(ike_header_t);
    uint8_t sa_payload[] = { 0x00, 0x00, 0x00, 0x01 }; 
    uint16_t p_len = sizeof(sa_payload);
    memcpy(payload, sa_payload, p_len);
    build_header(hdr, IKE_MSG_SA_PROPOSE, ctx->cky_i, ctx->cky_r, p_len);
